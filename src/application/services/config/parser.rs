...
fn confy_error_adapter(error: ConfyError) -> ConfigError {
    match error {
        ...
        ConfyError::BadConfigDirectory(error) => ConfigError::BadFormat(error.to_string()),
...
pub fn parse_config(config_path: Option<&String>) -> Result<Config, ConfigError> {
    let config = config_path
        .map_or_else(|| confy::load(NAME, None), |path| confy::load_path(path))
        .map_err(|error| confy_error_adapter(error))?;
    check_empty_pipeline_specification(&config).map(|_| config)
}

pub fn get_get_default_config_path() -> Result<PathBuf, ConfigError> {
    confy::get_configuration_file_path(NAME, None).map_err(|error| confy_error_adapter(error))
}
